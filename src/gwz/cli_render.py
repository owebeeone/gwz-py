...
def render_error(error: BaseException, *, json_mode: bool = False) -> str:
    if json_mode:
        ...
        if operation_errors:
            errors = [_merge_error_json(item) for item in operation_errors]
        else:
            errors = [_exception_error_json(error)]
        return json.dumps(
            {
                "kind": "response",
                "meta": None,
...
def _exception_error_json(error: BaseException) -> dict[str, Any]:
    ...
    return {
        ...
        "target_kind": _enum_label(target_kind) if target_kind is not None else None,
